...
class CagnotteAddAccountToRankBlock(Block):
    ...
    account_to_add = attrib = attrib(type=AccountId, converter=AccountId)
...
class CagnotteRemoveAccountToRankBlock(Block):
    ...
    user_id = attrib(type=UserId, converter=UserId)
    rank_name = attrib(type=str)
    account_to_remove = attrib = attrib(type=AccountId, converter=AccountId)
...
class UseService(Block):
    ...
    def execute(self, db: SwagChain):
        ...
        if service.authorized_rank is not None:
            if (
                self.user_id not in cagnotte.getRanklist(service.authorized_rank)
                or self.user_id
                in cagnotte.managers  # les managers font ce qu'ils veulent
            ):
