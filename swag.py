...
class SwagBank:
    ...
    class account:
        ...
        def __init__(self,balance=0, history=None, last_time_mining=date.min, date_of_creation=date.today()):
        ...
        def write_in_history(self,h_movement,account_name,value):
            ...
            """
            self.history.insert(0,(h_movement,account_name,value)) #Une transaction est représenté par un tuple
    ...
    class accountV2(account):
        ...
        def __init__(self,accountV1):
            ...
            """
            super().__init__(accountV1.balance,accountV1.history.copy(),accountV1.last_time_mining,accountV1.date_of_creation) #Construction 
            self.balanceStyle = 0
            self.blockedSwag = 0
    ...
    def __init__(self):
        ...
        """
        try:
            self.swagAccounts = pickle.load(open("bank.swag", "rb")) #On essaye d'ouvrir le fichier contenant l'ensemble des comptes
            for key_account_name, account in self.swagAccounts.items():
                if type(account) is self.account: #Si il reste des comptes de type V1 dans le dictionnaire alors :
                    self.swagAccounts[key_account_name] = self.accountV2(accountV1=account) #Mise à jour du compte 1 vers le compte 2

            self.saveAccounts()

        except (OSError, IOError) as e: #Si le fichier des comptes n'existe pas, on crée un dictionnaire de compte vide, et on crée le fichier des comptes.
            self.swagAccounts = {}
        ...
        if self.swagAccounts: ## On regarde si le dictionnaire des comptes n'est pas vide, si c'est le cas, on met à jour le plus swag.
    ...
    def moveMoney(self,account_name,howmany):
        ...
        """
        if account_name in self.swagAccounts.keys():
            self.swagAccounts[account_name].balance = self.swagAccounts[account_name].balance+howmany
            self.saveAccounts()
        else:
            raise NoAccountRegistered(account_name)
    ...
    def giveSwag(self,expeditor_account_name,destinator_account_name,valueToGive):
        ...
        self.swagAccounts[expeditor_account_name].write_in_history(self.account.history_movement.GIVE_TO,destinator_account_name,valueToGive)
        self.swagAccounts[destinator_account_name].write_in_history(self.account.history_movement.RECIEVE_FROM,expeditor_account_name,valueToGive)
        self.saveAccounts()
    ...
    def mine(self,account_name):
        ...
        """
        self.moveMoney(account_name,0) #petit tricks pour vérifier si le compte existe bien

        #On ne peut miner qu'une fois par jour
        if not self.swagAccounts[account_name].last_time_mining < date.today(): # On vérifie si la date de la dernière fois qu'on a miné est bien inférieure à la date d'aujourd'hui
            raise AlreadyMineToday

        mining_booty = cauchy.roll(SWAG_BASE,SWAG_LUCK) # Génération d'un nombre aléatoire, en suivant une loi de Cauchy
        self.moveMoney(account_name,mining_booty) # Ajout de cet argent au compte
        self.swagAccounts[account_name].last_time_mining = date.today() #Mise à jour de la date du dernier minage
        self.swagAccounts[account_name].write_in_history(self.account.history_movement.RECIEVE_FROM,"$wag Mine ⛏",mining_booty) # écriture dans l'historique
        self.saveAccounts()
    ...
    def getClassement(self,premier_en_premier = True):
        ...
        """
        return {key: account.balance for key, account in sorted(self.swagAccounts.items(), key=lambda item: item[1].balance, reverse=premier_en_premier)}
...
############################## Fonctions ajouté pour le $tyle ################################
    ...
    def getStyleTotalGrowthRate(self,account_name):
        ...
        """
        if account_name in self.swagAccounts.keys():
            return self.swagAccounts[account_name].styleGrowthRate + self.swagAccounts[account_name].styleBonusRate
        else:
            raise NoAccountRegistered(account_name)
    ...
    def getDateOfUnblockingSwag(self,account_name):
        ...
        if account_name in self.swagAccounts.keys():
            ...
            return self.swagAccounts[account_name].dateAndTimeOfBlockage + blocking_duration
        else:
            raise NoAccountRegistered(account_name)
    ...
    def moveStyle(self,account_name,howmany): #peut être utiliser pour le négatif
        ...
        """
        if account_name in self.swagAccounts.keys():
            self.swagAccounts[account_name].balanceStyle = self.swagAccounts[account_name].balanceStyle+howmany
            self.saveAccounts()
        else:
            raise NoAccountRegistered(account_name)
    ...
    def giveStyle(self,expeditor_account_name,destinator_account_name,valueToGive):
        ...
        #Write transaction in history
        self.swagAccounts[expeditor_account_name].write_in_history(self.account.history_movement.GIVE_TO,destinator_account_name,valueToGive,currency="$tyle")
        self.swagAccounts[destinator_account_name].write_in_history(self.account.history_movement.RECIEVE_FROM,expeditor_account_name,valueToGive,currency="$tyle")
        self.saveAccounts()
    ...
    def blockSwagToGetStyle(self,account_name,amount_of_swag):
        ...
        self.swagAccounts[account_name].write_in_history(self.accountV2.history_movement.GIVE_TO,"$tyle Generator Inc.",amount_of_swag,currency="$wag")
        self.saveAccounts()
    ...
    def updateBonusGrowthRate(self):
        ...
        for account_name in classement.keys():
            self.swagAccounts[account_name].styleBonusRate = round((10/3)*(1-pow(16,x / nbr_partcipant)),2) #Fonction mathématique, qui permet au premier d'avoir toujours 50%, et à celui à la moitié du classement 10%
            x+=1
        self.saveAccounts()
    ...
    def earnStyle(self,account_name):
        ...
        Args:
            account_name (String): Nom de compte, souvent le nom d'utilisateur général (ex : GlichiKun#4059)
        """
        block_booty = (self.getBlokedSwag(account_name) * SWAG_STYLE_RATIO * (self.getStyleTotalGrowthRate(account_name)*0.01)) / (TIME_OF_BLOCK*24) #division pour gagner toute les heures
        self.moveStyle(account_name,block_booty)
        self.swagAccounts[account_name].write_in_history(self.accountV2.history_movement.RECIEVE_FROM,"$tyle Generator Inc.",block_booty,currency="$tyle")
        self.saveAccounts()
    ...
    def deblockSwag(self,account_name):
        ...
        Raises:
            StyleStillBlocked: Levée si l'utilisateur a toujours du $wag bloqué
        """
        if datetime.now() < self.getDateOfUnblockingSwag(account_name): #Nous sommes toujours dans la période de blocage
            raise StyleStillBlocked
        ...
        self.swagAccounts[account_name].write_in_history(self.accountV2.history_movement.RECIEVE_FROM,"$tyle Generator Inc.",returned_swag,currency="$wag")
        self.saveAccounts()
